...
export default function UserComparison() {
  ...
  const extractZipFiles = async (zipFile: File) => {
    ...
    for (const [path, file] of Object.entries(loaded.files)) {
      if (path.endsWith(".json") && !file.dir) {
        ...
        const jsonFile = new File([content], path, {
  ...
  const extractUsersFromCsv = async (
    file: File,
    columnName: string,
    key: string
  ): Promise<Set<string>> => {
  ...
  const processJsonFiles = async (
    ...
    csvUsers: Set<string>,
    key: string
  ): Promise<{ matchingCount: number; missing: MissingUser[] }> => {
  ...
  const handleCompare = async () => {
    ...
    try {
      ...
      const { matchingCount, missing } = await processJsonFiles(
        jsonFiles,
        csvUsers,
        jsonKey
      );

      const uniqueMissingUsers = new Set(missing.map((m) => m.currentUser));
      ...
      setResults({
        ...
        csvUsers,
        uniqueMissingCount: uniqueMissingUsers.size,
      });
    } catch (err) {
      setError(
          ...
          <div className="space-y-4 mb-6">
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-2">
                Select the field to compare both lists
            ...
            {results.missingUsers.length === 0 && (
              <div className="text-center py-8 bg-green-50 rounded-lg border border-green-200">
                ...
                <p className="text-green-800 font-semibold">
                  All users matched successfully!
                </p>
                <p className="text-green-600 text-sm mt-1">
                  No missing users found in the comparison.
                </p>
              </div>
            )}
          </div>
